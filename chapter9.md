...
### fasttext 실습
- word2vec 과의 차이점?
  - word2vec 이후에 나온, 사실상 word2vec의 확장판
  - 글자 단위 n-gram 구성 취급. N 지정에 따라 하나의 단어가 N개로 분리될 수 있음.
    - 모르는 단어(Out Of Vocabulary: OOV) 에 대해 대응할 수 있으며, 등장 빈도 수가 적을 때 임베딩 정확도가 낮은 word2vec 의 단점 개선
    - 사이드 프로젝트 규모에서 진행하기 좋은 방법이 아닐까 싶다. 
- 네이버 영화 리뷰 실습
  - Word2vec 코드 대신 `from gensim.models import FastText`, `model = FastText(sentences=result, vector_size=100, window=5, min_count=5, workers=4, sg=1)` 삽입
    - Q. FastText 에서의 sg 는 무엇을 의미할까? Word2vec에서는 cbow, skip-gram을 의미했었다.
    - A. 마찬가지로 cbow, skip-gram 을 의미한다고 한다. -> 사이드 프로젝트에 적용할 때는 cbow, skip-gram 둘 다 해보고, 유사도가 더 정확한 친구를 골라야 할듯.
- word2vec 의 경우: 에러 반환 - "Key 'ironman' not present in vocabulary"
- fasttext의 경우: 일단 예측 진행. 
  ```text
  [('hoffman', 0.8265944123268127), ('ekman', 0.8169727325439453), ('craftsman', 0.8123198747634888), ('shaman', 0.8054773211479187), ('stuntman', 0.8033562302589417), ('wurman', 0.8020294904708862), ('odysseus', 0.8012235760688782), ('housman', 0.7975673675537109), ('kahneman', 0.79323410987854), ('kogi', 0.7921972274780273)]
  ```
- 한국어 OOV 처리도 가능. 한글의 자모를 처리하는 패키지인 hgtk 를 이용한다.
